...
function drawState (ctx, status) {
  ...
  var width = ctx.measureText(status).width
  console.error('width:', ctx.canvas.width, width, ctx.canvas.width - width)
  ctx.fillText(status, ctx.canvas.width - width, textHeight)
  ctx.translate(0, textHeight + 6)
...
var badge = module.exports = function (ctx, iteration) {
  ...
  var height = ~~(ctx.canvas.height/5)
  ctx.font = height+'px sans'

  drawTitle(ctx, iteration)
  drawState(ctx, iteration.status) 
  //state = states.indexOf(iteration.status), states)
  var r

  drawBar(ctx, r = iteration.sum/iteration.target, width,
    iteration.sum.toPrecision(3) + '(/' + (+iteration.target).toPrecision(3) + ') '
    + percent(r)
  )

  var lh = ~~(ctx.canvas.height/6)
  ...
  ctx.textAlign = 'right'
  ctx.fillStyle = '#333333'
  ctx.fillText('FEEDOPENSOURCE', width, lh*1.4)
